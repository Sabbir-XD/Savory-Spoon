import { FaStar, FaRegStar, FaShoppingCart, FaInfoCircle, FaFire, FaLeaf, FaHeart } from 'react-icons/fa';
...
const FoodCard = ({ food }) => {
  ...
  const getCategoryIcon = () => {
    if (food.category === 'spicy') return <FaFire className="text-red-500 dark:text-red-400" />;
    if (food.category === 'vegetarian') return <FaLeaf className="text-green-500 dark:text-green-400" />;
    return null;
  };
  ...
  return (
    <div 
      className={`relative bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 ease-in-out 
        ${isHovered ? 'transform -translate-y-2 shadow-xl dark:shadow-gray-700/50' : 'shadow-md dark:shadow-gray-900'}`}
    ...
    >
      ...
      <button 
        className={`absolute top-3 left-3 z-10 p-2 rounded-full transition-all 
          ${isFavorite ? 'text-red-500 dark:text-red-400 bg-white/90 dark:bg-gray-800/90' : 
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <span className="text-amber-600 dark:text-amber-400 font-bold text-xl">Tk{food.price}</span>
            {food.originalPrice && (
